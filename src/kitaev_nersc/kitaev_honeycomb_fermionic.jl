...
function get_simulation_params()
  ...
  return (
    ...
    η₁=2.0, η₂=2.0                # Perturbation strengths for the operators on the edges of the cylinder to extract fermionic binding energy
  )
end
...
let
  ...
  for (label, value) in pairs((; Jx, Jy, Jz, kappa, t, P, λ₁, λ₂, η₁, η₂))
  ...
  """
    Set up chemical potential on the edges to confine the hole in the bulk of the cylinder
  """
  ...
  """
    Add edge operators to extract the fermionic binding energy 
  """
  # Define edge sites for fermionic binding energy extraction
  # First 4 sites on left edge, last 4 sites on right edge
