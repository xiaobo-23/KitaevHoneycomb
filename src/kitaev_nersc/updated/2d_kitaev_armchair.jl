...
let
  ...
  @show typeof(edge_sites), length(edge_sites)
  ...
  begin
    os = OpSum()
    os = construct_two_body_interaction(os, lattice, t, Jx, Jy, Jz, Ny, number_of_bonds) # Two-body anisotropic Kitaev interaction
    os = construct_three_spin_interaction(os, wedge, kappa, Ny, number_of_wedges) # Three-spin interaction terms
    os = construct_edge_potential(os, edge_sites, P)  # Edge chemical potential to confine the hole in the bulk# 
    os = construct_string_potential(os, λ₁, λ₂, string_potential, Nx, Ny, N)  # String potential to prevent the skewness of electron density
  end
