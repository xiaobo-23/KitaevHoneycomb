# 08/07/2024
# Simulate the Kitaev model on the honeycomb lattice with a cylinder geometry
# Introduce vacancies, magnetic field, plaquette perturbations, and loop perturbations. 
...
let
  ...
  # Set up the strength of the loop and plaquette perturbations
  lambda_left=0
  lambda_right=0
  eta=abs(lambda_left)                                          # Use a positive sign here to lower the energy, given the plaquette operator is negative
  @show Jx, Jy, Jz, alpha, h, lambda_left, lambda_right, eta
  ...
  if abs(lambda_left) > 1E-8
    for index in 1 : Int(size(pinning_sites, 1) / 2)
      @show pinning_sites[index, 1], lambda_left
  ...
  if abs(lambda_right) > 1E-8
    for index in Int(size(pinning_sites, 1) / 2) + 1 : size(pinning_sites, 1)
      @show pinning_sites[index, 1], lambda_right
  ...
  # println("")
  # tmp_plaquette_indices = plaquette_indices[setdiff(1 : size(plaquette_indices, 1), range(16, 26, step = 1)), :]
